...
export default function FeaturesGrid() {
  return (
    ...
    >
      ...
      <div style={{ pointerEvents: 'none', position: 'absolute', left: '20%', bottom: '10%', width: 500, height: 500, borderRadius: '50%', background: 'rgba(56,189,248,0.05)', filter: 'blur(120px)' }} />
      ...
      <div style={{
        ...
        <motion.div
          ...
          style={{ maxWidth: 700, marginLeft: 'auto', marginRight: 'auto', marginBottom: 104, textAlign: 'center' }}
        >
          ...
          <h2 style={{ fontSize: 'clamp(2.6rem, 5.5vw, 4.2rem)', fontWeight: 900, lineHeight: 1.06, letterSpacing: '-0.04em', margin: 0, color: '#f1f5f9' }}>
          ...
          <p style={{ marginTop: 22, fontSize: 18, lineHeight: 1.8, color: '#94a3b8', maxWidth: 540, marginLeft: 'auto', marginRight: 'auto' }}>
            A live look at the AI-powered modules, dashboards, and workflows already inside the platform.
          </p>
        </motion.div>

        {/* ── ALTERNATING FEATURE ROWS ── */}
        {/* Layout: 40% text | 60% screen — flips every row */}
          ...
          {sections.map((s, i) => {
            ...
            return (
              <motion.div
                key={s.title}
                variants={fadeUp} initial="hidden" whileInView="visible"
                transition={{ duration: 0.65 }} viewport={{ once: true, amount: 0.1 }}
                style={{
                  display: 'grid',
                  gridTemplateColumns: flip ? '62% 38%' : '38% 62%',
              ...
              >
                ...
                <div style={{ order: flip ? 2 : 1, display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                  <div style={{ width: 64, height: 64, borderRadius: 18, border: '1px solid rgba(103,232,249,0.22)', background: 'rgba(103,232,249,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 22, boxShadow: '0 8px 32px rgba(47,230,255,0.1)' }}>
                    <Icon size={30} color="#67e8f9" />
                  </div>
                  ...
                  <h3 style={{ fontSize: 'clamp(2rem, 3.4vw, 3rem)', fontWeight: 900, lineHeight: 1.08, letterSpacing: '-0.035em', color: '#f1f5f9', margin: 0 }}>{s.title}</h3>
                  <p style={{ marginTop: 18, fontSize: 17, lineHeight: 1.8, color: '#94a3b8', maxWidth: 420 }}>{s.description}</p>
                  <a
                    href="#"
                    style={{ marginTop: 30, display: 'inline-flex', alignItems: 'center', gap: 8, borderRadius: 999, border: '1px solid rgba(103,232,249,0.25)', background: 'rgba(103,232,249,0.08)', padding: '11px 22px', fontSize: 13, fontWeight: 700, color: '#67e8f9', textDecoration: 'none', transition: 'background 0.2s, transform 0.2s' }}
        ...
        <div style={{ marginTop: 180 }}>
          ...
          <motion.div
            ...
            style={{ maxWidth: 700, marginLeft: 'auto', marginRight: 'auto', marginBottom: 72, textAlign: 'center' }}
          >
            ...
            {toolCategories.map((cat, ci) => {
              ...
              return (
                <motion.div
                  ...
                  className={`group relative overflow-hidden rounded-2xl border bg-white/[0.025] p-8 shadow-lg transition-all duration-300 hover:shadow-2xl hover:bg-white/[0.04] ${p.card}`}
                >
                  {/* hover glow */}
                    ...
                    {cat.tools.map((t) => {
                      ...
                      return (
                        <a
                          key={t.name}
                          href="/app/tools"
                          className={`group/row flex items-center gap-4 rounded-xl border border-white/[0.06] bg-white/[0.03] px-5 py-4 transition duration-200 ${p.row}`}
                          style={{ textDecoration: 'none' }}
                        >
                          <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${p.icon}`}>
                            <TIcon size={21} />
                          </div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ fontSize: 14, fontWeight: 700, lineHeight: 1.3, color: '#f1f5f9', margin: 0 }}>{t.name}</p>
                            <p style={{ fontSize: 12, lineHeight: 1.6, color: '#64748b', margin: '4px 0 0' }}>{t.desc}</p>
                          </div>
...
function ScreenPreview({ src, alt }: { src: string; alt: string }) {
  return (
    ...
    >
      {/* outer glow */}
      <div
        style={{ position: 'absolute', inset: -20, borderRadius: 36, background: 'linear-gradient(135deg,rgba(47,230,255,0.14),transparent 50%,rgba(122,60,255,0.10))', filter: 'blur(28px)', opacity: 0.55, pointerEvents: 'none', transition: 'opacity 0.5s' }}
        className="group-hover:opacity-90"
      />

      {/* browser shell */}
      <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 20, border: '1px solid rgba(255,255,255,0.09)', background: '#080f22', boxShadow: '0 48px 120px rgba(0,0,0,0.65), 0 0 0 1px rgba(255,255,255,0.04) inset' }}>

        {/* browser chrome bar */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, height: 44, padding: '0 18px', borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.02)', flexShrink: 0 }}>
          <span style={{ width: 13, height: 13, borderRadius: '50%', background: 'rgba(255,95,87,0.85)',   display: 'block', flexShrink: 0 }} />
          <span style={{ width: 13, height: 13, borderRadius: '50%', background: 'rgba(255,189,46,0.85)',  display: 'block', flexShrink: 0 }} />
          <span style={{ width: 13, height: 13, borderRadius: '50%', background: 'rgba(40,202,66,0.85)',   display: 'block', flexShrink: 0 }} />
          <span style={{ marginLeft: 12, width: 180, height: 13, borderRadius: 7, background: 'rgba(255,255,255,0.06)', display: 'block' }} />
        </div>
        ...
        <div style={{ overflow: 'hidden', lineHeight: 0 }}>
          <Image
            ...
            quality={100}
            priority={false}
            sizes="(min-width: 1360px) 820px, (min-width: 768px) 62vw, 100vw"
            style={{
              display: 'block',
              width: '100%',
              aspectRatio: '16/10',
              ...
              transition: 'transform 0.75s ease, object-position 0.75s ease',
              willChange: 'transform',
            }}
            className="group-hover:scale-[1.05] group-hover:[object-position:center_18%]"
          />
        </div>
      </div>
    </motion.div>
